...
object StudentWeekly {
  def main(args: Array[String]): Unit = {
    ...
    val str_create_swqw=s""" create table test.school_weekly_question_week as
                           ...
                           |join (
                           |select org_id,case when private=0 then 2 else 4 end org_type  from neworiental_logdata.entity_school where enable=1
                           |    union all
    ...
    val str_create_sws=s""" create table test.student_weekly_survey as
                          ...
                          |	h.org_type public_private,h.exercise_source pointer_type,h.subject_id,
                          ...
                          |(
                          |    -----所有年级,所有学科,全部pad
                          |	select org_id,org_type,0 grade_id,0 subject_id,class_id,exercise_source,student_id,9999 pad_class,
                          ...
                          |	group by org_id,org_type,class_id,exercise_source,student_id
                          |	union all
                          |	--所有指标
                          |	select org_id,org_type,0 grade_id,0 subject_id,class_id,0 exercise_source,student_id,9999 pad_class,
                          |	count(question_id) answer_num,
                          |	((sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end))/count(question_id)) answer_lv,
                          |	sum(case when ret_num=1 then 1
                          ...
                          |	sum(case when ret_num=6 then 1 else 0 end) unknown_num
                          |	from test.school_weekly_question_week
                          |	group by org_id,org_type,class_id,student_id
                          |	union all
                          |	-----所有年级,所有学科,分pad
                          |	select org_id,org_type,0 grade_id,0 subject_id,class_id,exercise_source,student_id,pad_class,
                          |	count(question_id) answer_num,
                          |	((sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end))/count(question_id)) answer_lv,
                          |	sum(case when ret_num=1 then 1
                          ...
                          |	sum(case when ret_num=6 then 1 else 0 end) unknown_num
                          |	from test.school_weekly_question_week
                          |	group by org_id,org_type,pad_class,class_id,exercise_source,student_id
                          |	union all
                          |	--所有指标
                          |	select org_id,org_type,0 grade_id,0 subject_id,class_id,0 exercise_source,student_id,pad_class,
                          |	count(question_id) answer_num,
                          |	((sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end))/count(question_id)) answer_lv,
                          |	sum(case when ret_num=1 then 1
                          ...
                          |	sum(case when ret_num=6 then 1 else 0 end) unknown_num
                          |	from test.school_weekly_question_week
                          |	group by org_id,org_type,pad_class,class_id,student_id
                          |	union all
                          |	-----各个年级,所有学科,全部pad
                          |	select org_id,org_type,grade_id,0 subject_id,class_id,exercise_source,student_id,9999 pad_class,
                          |	count(question_id) answer_num,
                          |	((sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end))/count(question_id)) answer_lv,
                          |	sum(case when ret_num=1 then 1
                          ...
                          |	sum(case when ret_num=6 then 1 else 0 end) unknown_num
                          |	from test.school_weekly_question_week
                          |	group by org_id,org_type,grade_id,class_id,exercise_source,student_id
                          |	union all
                          |	--所有指标
                          |	select org_id,org_type,grade_id,0 subject_id,class_id,0 exercise_source,student_id,9999 pad_class,
                          |	count(question_id) answer_num,
                          |	((sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end))/count(question_id)) answer_lv,
                          |	sum(case when ret_num=1 then 1
                          ...
                          |	sum(case when ret_num=6 then 1 else 0 end) unknown_num
                          |	from test.school_weekly_question_week
                          |	group by org_id,org_type,grade_id,class_id,student_id
                          |	union all
                          |	--各个年级,所有学科,分pad
                          |	select org_id,org_type,grade_id,0 subject_id,class_id,exercise_source,student_id,pad_class,
                          |	count(question_id) answer_num,
                          |	((sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end))/count(question_id)) answer_lv,
                          |	sum(case when ret_num=1 then 1
                          ...
                          |	sum(case when ret_num=6 then 1 else 0 end) unknown_num
                          |	from test.school_weekly_question_week
                          |	group by org_id,org_type,grade_id,pad_class,class_id,exercise_source,student_id
                          |	union all
                          |	--所有指标
                          |	select org_id,org_type,grade_id,0 subject_id,class_id,0 exercise_source,student_id,pad_class,
                          |	count(question_id) answer_num,
                          |	((sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end))/count(question_id)) answer_lv,
                          |	sum(case when ret_num=1 then 1
                          ...
                          |	sum(case when ret_num=6 then 1 else 0 end) unknown_num
                          |	from test.school_weekly_question_week
                          |	group by org_id,org_type,grade_id,pad_class,class_id,student_id
                          |	union all
                          |	--各个年级,各个学科,全部pad
                          |	select org_id,org_type,grade_id,subject_id,class_id,exercise_source,student_id,9999 pad_class,
                          |	count(question_id) answer_num,
                          |	((sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end))/count(question_id)) answer_lv,
                          |	sum(case when ret_num=1 then 1
                          ...
                          |	sum(case when ret_num=6 then 1 else 0 end) unknown_num
                          |	from test.school_weekly_question_week
                          |	group by org_id,org_type,grade_id,subject_id,class_id,exercise_source,student_id
                          |	union all
                          |	--所有指标
                          |	select org_id,org_type,grade_id,subject_id,class_id,0 exercise_source,student_id,9999 pad_class,
                          |	count(question_id) answer_num,
                          |	((sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end))/count(question_id)) answer_lv,
                          |	sum(case when ret_num=1 then 1
                          ...
                          |	sum(case when ret_num=6 then 1 else 0 end) unknown_num
                          |	from test.school_weekly_question_week
                          |	group by org_id,org_type,grade_id,subject_id,class_id,student_id
                          |	union all
                          |	--各个年级,各个学科,分pad
                          |	select org_id,org_type,grade_id,subject_id,class_id,exercise_source,student_id,pad_class,
                          |	count(question_id) answer_num,
                          |	((sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end))/count(question_id)) answer_lv,
                          |	sum(case when ret_num=1 then 1
                          ...
                          |	sum(case when ret_num=6 then 1 else 0 end) unknown_num
                          |    from test.school_weekly_question_week
                          |	group by org_id,org_type,grade_id,subject_id,pad_class,class_id,exercise_source,student_id
                          |	union all
                          |	--所有指标
                          |	select org_id,org_type,grade_id,subject_id,class_id,0 exercise_source,student_id,pad_class,
                          |	count(question_id) answer_num,
                          |	((sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end))/count(question_id)) answer_lv,
                          |	sum(case when ret_num=1 then 1
                          ...
                          |	sum(case when ret_num=6 then 1 else 0 end) unknown_num
                          |    from test.school_weekly_question_week
                          |	group by org_id,org_type,grade_id,subject_id,pad_class,class_id,student_id
                          |) h) h1 """.stripMargin
    sql(str_create_sws)
    ...
    val str_create_swd=s""" create table test.student_weekly_details as
                          |select cast(concat('${DAY}',row_number() over()) as bigint) id,h1.* from
                          |(   select distinct h.org_id,h.grade_id,h.class_id,h.student_id system_id,h.org_type public_private,h.subject_id,
                          ...
                          |	year('${DATE6AGO}') leran_year, weekofyear('${DATE}') leran_week,h.pad_class is_pad,'${DATE}' time
                          |	from
                          |(	select org_id,org_type,0 grade_id,0 subject_id,class_id,student_id,9999 pad_class,
                          ...
                          |	union all
                          |	-----所有年级，pad
                          |	select org_id,org_type,0 grade_id,0 subject_id,class_id,student_id,pad_class,
                          |	count(question_id) all_num,
                          |	sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end) right_num,
                          |	sum(case when exercise_source not in (1,6,7,8) then 1 else 0 end) zz_num,
                          ...
                          |	union all
                          |	--各个年级
                          |	select org_id,org_type,grade_id,0 subject_id,class_id,student_id,9999 pad_class,
                          |	count(question_id) all_num,
                          |	sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end) right_num,
                          |	sum(case when exercise_source not in (1,6,7,8) then 1 else 0 end) zz_num,
                          ...
                          |	union all
                          |	--各个年级，pad
                          |	select org_id,org_type,grade_id,0 subject_id,class_id,student_id,pad_class,
                          |	count(question_id) all_num,
                          |	sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end) right_num,
                          |	sum(case when exercise_source not in (1,6,7,8) then 1 else 0 end) zz_num,
                          ...
                          |	union all
                          |	---各个年级学科
                          |	select org_id,org_type,grade_id,subject_id,class_id,student_id,9999 pad_class,
                          |	count(question_id) all_num,
                          |	sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end) right_num,
                          |	sum(case when exercise_source not in (1,6,7,8) then 1 else 0 end) zz_num,
                          ...
                          |	union all
                          |	---各个年级学科，pad
                          |	select org_id,org_type,grade_id,subject_id,class_id,student_id,pad_class,
                          |	count(question_id) all_num,
                          |	sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end) right_num,
                          |	sum(case when exercise_source not in (1,6,7,8) then 1 else 0 end) zz_num,
