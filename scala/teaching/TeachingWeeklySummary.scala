...
object TeachingWeeklySummary {
  def main(args: Array[String]): Unit = {
    ...
    val str_create_cds=""" create  table  test.class_daily_summary as
                         |			select org_id,0 grade_id, 0 class_id,9999 pad_class,count(distinct class_id) class_daily_num,time
                         |			from  test.class_daily where class_daily=1
                         |			group by time,org_id
                         |			union all
                         |			select org_id,0 grade_id, 0 class_id,pad_class,count(distinct class_id) class_daily_num,time
                         |			from  test.class_daily where class_daily=1
                         |			group by time,org_id,pad_class
                         |			union all
                         |			select org_id,grade_id, 0 class_id,9999 pad_class,count(distinct class_id) class_daily_num,time
                         |			from  test.class_daily where class_daily=1
                         |			group by time,org_id,grade_id
                         |			union all
                         |			select org_id,grade_id, 0 class_id,pad_class,count(distinct class_id) class_daily_num,time
                         |			from  test.class_daily where class_daily=1
                         |			group by time,org_id,grade_id,pad_class
                         |			union all
                         |			select org_id,grade_id,class_id, 9999 pad_class,count(distinct class_id) class_daily_num,time
                         |			from  test.class_daily where class_daily=1
                         |			group by time,org_id,grade_id,class_id
                         |			union all
                         |			select org_id,grade_id,class_id, pad_class,count(distinct class_id) class_daily_num,time
                         |			from  test.class_daily where class_daily=1
                         |			group by time,org_id,grade_id,class_id,pad_class """.stripMargin
    sql(str_create_cds)
    /*APP用户汇总*/
    ...
    val str_create_saus=""" create table test.student_app_user_summary as
                          ...
                          |'9999-12-31' time
                          |from test.student_app_user
                          |group by org_id,grade_id,pad_class """.stripMargin
    sql(str_create_saus)
    /*学生活跃汇总*/
    ...
    val str_create_sds=""" create table test.student_daily_summary as
                         |		select org_id,0 grade_id,0 class_id,9999 pad_class,count(distinct system_id) studnet_daily_num,time from test.student_daily
                         |		group by time,org_id
                         |		union all
                         |		select org_id,0 grade_id,0 class_id,pad_class,count(distinct system_id) studnet_daily_num,time from test.student_daily
                         |		group by time,org_id,pad_class
                         |		union all
                         |		select org_id,grade_id,0 class_id,9999 pad_class,count(distinct system_id) studnet_daily_num,time from test.student_daily
                         |		group by time,org_id,grade_id
                         |		union all
                         |		select org_id,grade_id,0 class_id,pad_class,count(distinct system_id) studnet_daily_num,time from test.student_daily
                         |		group by time,org_id,grade_id,pad_class
                         |		union all
                         |		select org_id,grade_id,class_id,9999 pad_class,count(distinct system_id) studnet_daily_num,time from test.student_daily
                         |		group by time,org_id,grade_id,class_id
                         |		union all
                         |		select org_id,grade_id,class_id,pad_class,count(distinct system_id) studnet_daily_num ,time from test.student_daily
                         |		group by time,org_id,grade_id,class_id,pad_class """.stripMargin
    sql(str_create_sds)
    /*学生答题汇总*/
    ...
    val str_create_sqs=""" create table test.student_question_summary as
                         |      select org_id,0 grade_id,0 class_id,9999 pad_class,count(question_id) answer_num,time from test.student_question
                         |			where exercise_source in (1,6,7,8)
                         |      group by time,org_id
                         |			union all
                         |			select org_id,0 grade_id,0 class_id,pad_class,count(question_id) answer_num,time from test.student_question
                         |			where exercise_source in (1,6,7,8)
                         |      group by time,org_id,pad_class
                         |			union all
                         |			select org_id,grade_id,0 class_id,9999 pad_class,count(question_id) answer_num,time from test.student_question
                         |			where exercise_source in  (1,6,7,8)
                         |      group by time,org_id,grade_id
                         |			union all
                         |			select org_id,grade_id,0 class_id,pad_class,count(question_id) answer_num,time from test.student_question
                         |			where exercise_source in (1,6,7,8)
                         |      group by time,org_id,grade_id,pad_class
                         |			union all
                         |			select org_id,grade_id,class_id,9999 pad_class,count(question_id) answer_num,time from test.student_question
                         |			where exercise_source in (1,6,7,8)
                         |      group by time,org_id,grade_id,class_id
                         |			union all
                         |			select org_id,grade_id,class_id,pad_class,count(question_id) answer_num,time from test.student_question
                         |			where exercise_source in (1,6,7,8)
                         |      group by time,org_id,grade_id,class_id,pad_class """.stripMargin
    sql(str_create_sqs)
    /*学生各场景答题汇总*/
    ...
    val str_create_sss=""" create table test.student_scence_summary as
                         |select h.org_id,h.grade_id,h.pad_class,
                         ...
                         |'9999-12-31' time
                         |from test.student_question
                         |group by org_id,grade_id,pad_class
                         |) h """.stripMargin
    sql(str_create_sss)
    /*学生各学科答题汇总*/
    ...
    val str_create_ssas="""  create  table test.student_subject_answer_summary as
                          |			select utn.org_id org_id,0 grade_id,9999 pad_class,
                          ...
                          |			group by  utn.org_id,utn.subject_name,utn.system_id
                          |
                          |			union all
                          |			select utn.org_id org_id,0 grade_id,utn.pad_class pad_class,
                          |					utn.subject_name subject_name,utn.system_id system_id,
                          |					sum(uts.num) student_num,
                          |					sum(case when sq.ques_num is not  null  then sq.ques_num else 0 end) subject_answer_num,
                          |					count(distinct utn.class_id) manage_class_num
                          ...
                          |					join test.class_student_num  uts on uts.class_id=utn.class_id
                          |					left join test.class_answer_num  sq on utn.class_id=sq.class_id and sq.subject_name=utn.subject_name
                          |			group by  utn.org_id,utn.pad_class,utn.subject_name,utn.system_id
                          |			union all
                          |			select utn.org_id org_id,utn.grade_id grade_id,9999 pad_class,
                          |					utn.subject_name subject_name,utn.system_id system_id,
                          |					sum(uts.num) student_num,
                          |					sum(case when sq.ques_num is not  null  then sq.ques_num else 0 end) subject_answer_num,
                          |					count(distinct utn.class_id) manage_class_num
                          ...
                          |					join test.class_student_num  uts on uts.class_id=utn.class_id
                          |					left join test.class_answer_num  sq on utn.class_id=sq.class_id and sq.subject_name=utn.subject_name
                          |			group by  utn.org_id,utn.grade_id,utn.subject_name,utn.system_id
                          |			union all
                          |			select utn.org_id org_id,utn.grade_id grade_id,utn.pad_class pad_class,
                          |			        utn.subject_name subject_name,utn.system_id system_id,
                          |					sum(uts.num) student_num,
                          |					sum(case when sq.ques_num is not  null  then sq.ques_num else 0 end) subject_answer_num,
                          |					count(distinct utn.class_id) manage_class_num
                          ...
                          |					join test.class_student_num  uts on uts.class_id=utn.class_id
                          |					left join  test.class_answer_num sq on utn.class_id=sq.class_id and sq.subject_name=utn.subject_name
                          |			group by  utn.org_id,utn.grade_id,utn.pad_class,utn.subject_name,utn.system_id  """.stripMargin
    sql(str_create_ssas)
    /*学生各学科各场景答题汇总*/
    ...
    val str_create_ssss="""  create table test.student_subject_scence_summary as
                          |select org_id,subject_name,0 grade_id,9999 pad_class,
                          ...
                          |from test.student_question
                          |group by org_id,subject_name
                          |union all
                          |select org_id,subject_name,0 grade_id,pad_class,
                          |count(distinct case when exercise_source=7 then student_id end) cp_user_num,
                          |count(case when exercise_source=7 then question_id end) cp_answer_num,
                          |
                          |count(distinct case when exercise_source=1 then student_id end) sk_user_num,
                          ...
                          |count(case when exercise_source=6 then question_id end) zy_answer_num
                          |from test.student_question
                          |group by org_id,subject_name,pad_class
                          |union all
                          |select org_id,subject_name,grade_id,9999 pad_class,
                          |count(distinct case when exercise_source=7 then student_id end) cp_user_num,
                          |count(case when exercise_source=7 then question_id end) cp_answer_num,
                          |
                          |count(distinct case when exercise_source=1 then student_id end) sk_user_num,
                          ...
                          |count(case when exercise_source=6 then question_id end) zy_answer_num
                          |from test.student_question
                          |group by org_id,subject_name,grade_id
                          |union all
                          |select org_id,subject_name,grade_id,pad_class,
                          |count(distinct case when exercise_source=7 then student_id end) cp_user_num,
                          |count(case when exercise_source=7 then question_id end) cp_answer_num,
                          |
                          |count(distinct case when exercise_source=1 then student_id end) sk_user_num,
                          ...
                          |count(case when exercise_source=6 then question_id end) zy_answer_num
                          |from test.student_question
                          |group by org_id,subject_name,grade_id,pad_class  """.stripMargin
    sql(str_create_ssss)
    /*学生答题(平均答题,管理班级数)汇总*/
    ...
    val str_create_sas=""" create table test.subject_answer_summary as
                         ...
                         |					join test.class_student_num uts on uts.class_id=utn.class_id
                         |					left join test.class_answer_num sq on utn.class_id=sq.class_id and sq.subject_name=utn.subject_name
                         |			group by utn.org_id,utn.subject_name
                         |			union all
                         |			select utn.org_id org_id,0 grade_id,utn.pad_class pad_class,
                         |					utn.subject_name subject_name,'无' uname,
                         |					sum(uts.num) student_num,
                         ...
                         |					join test.class_student_num  uts on uts.class_id=utn.class_id
                         |					left join test.class_answer_num sq on utn.class_id=sq.class_id and sq.subject_name=utn.subject_name
                         |			group by utn.org_id,utn.pad_class,utn.subject_name
                         |			union all
                         |			select utn.org_id org_id,utn.grade_id grade_id,9999 pad_class,
                         |					utn.subject_name subject_name,'无' uname,
                         |					sum(uts.num) student_num,
                         ...
                         |					join test.class_student_num  uts on uts.class_id=utn.class_id
                         |					left join test.class_answer_num sq on utn.class_id=sq.class_id and sq.subject_name=utn.subject_name
                         |			group by utn.org_id,utn.grade_id,utn.subject_name
                         |			union all
                         |			select utn.org_id org_id,utn.grade_id grade_id,utn.pad_class pad_class,
                         |					utn.subject_name subject_name,'无' uname,
                         |					sum(uts.num) student_num,
                         ...
                         |					join test.class_student_num uts on uts.class_id=utn.class_id
                         |					left join  test.class_answer_num sq on utn.class_id=sq.class_id and sq.subject_name=utn.subject_name
                         |			group by utn.org_id,utn.grade_id,utn.pad_class,utn.subject_name """.stripMargin
    sql(str_create_sas)
    /*老师各场景下发布*/
    ...
    val str_create_tsps=""" create table test.teacher_subject_publish_summary as
                         |			select org_id,system_id,subject_name,0 grade_id,9999 pad_class,
                         ...
                         |			group by org_id,subject_name,system_id
                         |			union all
                         |			select org_id,system_id,subject_name,0 grade_id,pad_class,
                         |						  sum(CASE WHEN pu_type=1 then 1   else 0 END) fb_sk_number,
                         |						  sum(CASE WHEN pu_type=6 then 1   else 0 END) fb_zy_number,
                         |						  sum(CASE WHEN pu_type=7 then 1   else 0 END) fb_cp_number,
                         |						  sum(CASE WHEN pu_type=8 then 1   else 0 END) fb_yx_number
                         |						  from test.teacher_publish
                         |			group by org_id,pad_class,subject_name,system_id
                         |			union all
                         |			select org_id,system_id,subject_name,grade_id,9999 pad_class,
                         |						  sum(CASE WHEN pu_type=1 then 1   else 0 END) fb_sk_number,
                         |						  sum(CASE WHEN pu_type=6 then 1   else 0 END) fb_zy_number,
                         |						  sum(CASE WHEN pu_type=7 then 1   else 0 END) fb_cp_number,
                         |						  sum(CASE WHEN pu_type=8 then 1   else 0 END) fb_yx_number
                         |						  from test.teacher_publish
                         |			group by org_id,grade_id,subject_name,system_id
                         |			union all
                         |			select org_id,system_id,subject_name,grade_id,pad_class,
                         |						  sum(CASE WHEN pu_type=1 then 1   else 0 END) fb_sk_number,
                         |						  sum(CASE WHEN pu_type=6 then 1   else 0 END) fb_zy_number,
                         |						  sum(CASE WHEN pu_type=7 then 1   else 0 END) fb_cp_number,
                         |						  sum(CASE WHEN pu_type=8 then 1   else 0 END) fb_yx_number
                         |						  from test.teacher_publish
                         |			group by org_id,grade_id,pad_class,subject_name,system_id """.stripMargin
    sql(str_create_tsps)
    sc_1.stop()
  }
}
