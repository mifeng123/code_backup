...
object ResourcePlatformLocal {
  def main(args: Array[String]): Unit = {
    ...
    val str_kejian=s""" --课件
                      ...
                      |select 1 type,t.code_id code_id,t.org_type,count(distinct t.num_id) num,
                      |    count( distinct  case when t.share_time>=date_sub(date_sub('${DATE}',1),pmod(datediff(date_sub('${DATE}',1),'1920-01-01')-3,7)-1) and t.share_time<'${DATE}' then t.num_id end) num_week,
                      |	count( distinct  case when t.share_time>=date_sub(date_sub('${DATE}',1),day('${DATE}')-2) and t.share_time<'${DATE}' then t.num_id end) month_week,
                      |    1 num_type,t.subject_id subject_id,t.stage_id stage_id from
                      |(
                      |select 1 type,lerm.org_id code_id,c.org_type org_type,c.id  num_id,share_time,
                      ...
                      |--初始化资源库
                      |union all
                      |select 1 type, prv.org_id code_id,prv.org_type org_type,c.id num_id,share_time,1 num_type,prv.subject_id subject_id,
                      |prv.stage_id stage_id from
                      |(select org_id,org_type,stage_id,subject_id,version_id from  test.link_resource_platform_version_config
                      ...
                      |--初始化资源库
                      |union all
                      |select 2 type, prv.director_code code_id,prv.org_type org_type,c.id num_id,share_time,1 num_type,prv.subject_id subject_id,
                      |prv.stage_id stage_id from
                      |(select director_code,org_type,stage_id,subject_id,version_id from  test.link_resource_platform_version_config
                      |lateral view explode(split(version_ids,',')) t as version_id
                      ...
                      |--初始化资源库
                      |union all
                      |select 3 type, prv.director_code code_id,prv.org_type org_type,c.id num_id,share_time,1 num_type,prv.subject_id subject_id,
                      |prv.stage_id stage_id from
                      |(select director_code ,org_type,stage_id,subject_id,version_id from  test.link_resource_platform_version_config
                      |lateral view explode(split(version_ids,',')) t as version_id
                      ...
                      |--初始化资源库
                      |union all
                      |select 4 type, prv.director_code code_id,prv.org_type org_type,c.id num_id,share_time,1 num_type,prv.subject_id subject_id,
                      |prv.stage_id  stage_id from
                      |(select director_code,org_type,stage_id,subject_id,version_id from  test.link_resource_platform_version_config
                      |lateral view explode(split(version_ids,',')) t as version_id
    ...
    val  str_ziliao=s""" insert into table test.resource_local_tmp
                       |select 1 type,t.code_id code_id,t.org_type org_type,count(distinct t.num_id) num,
                       |    count(distinct case when t.share_time>=date_sub(date_sub('${DATE}',1),pmod(datediff(date_sub('${DATE}',1),'1920-01-01')-3,7)-1) and t.share_time<'${DATE}' then  t.num_id end) num_week,
                       |    count( distinct  case when t.share_time>=date_sub(date_sub('${DATE}',1),day('${DATE}')-2) and t.share_time<'${DATE}' then t.num_id end) month_week,
                       |    t.num_type num_type,t.subject_id subject_id,t.stage_id stage_id from
                       |(
                       |select 1 type,c.code_id code_id,org_type,c.id num_id,share_time,
                       ...
                       |--初始化资源库
                       |union all
                       |select 1 type,c.code_id code_id,c.type_id org_type,c.id num_id,share_time,
                       |    c.num_type  num_type,c.ject_id subject_id,c.tag_id stage_id
                       |from (
                       |    select ec.*,prv.org_id code_id,prv.org_type type_id,prv.stage_id tag_id,prv.subject_id ject_id,
                       |        case when (ec.ext_id=0 or ec.ext_id is null) and ec.resource_tag=2 then 2
                       |             when (ec.ext_id=0 or ec.ext_id is null) and ec.resource_tag=3 then 3
                       |             when (ec.ext_id=0 or ec.ext_id is null) and ec.resource_tag=4 then 4
                       |             when (ec.ext_id=0 or ec.ext_id is null) and ec.resource_tag=5 then 5
                       ...
                       |			 when (ec.ext_id=0 or ec.ext_id is null) and ec.resource_tag=11 then 13
                       |             else 0 end num_type
                       |   from (select org_id,org_type,stage_id,subject_id,version_id from  test.link_resource_platform_version_config
                       |lateral view explode(split(version_ids,',')) t as version_id
                       |where director_type=1) prv
                       |join  neworiental_v3.entity_teaching_directory etd on etd.version_id =prv.version_id
                       |join  neworiental_v3.entity_teaching_chapter etc on etc.directory_id=etd.id
                       ...
                       |--初始化资源库
                       |union all
                       |select 2 type,c.code_id code_id,c.type_id org_type,c.id num_id,share_time,
                       |    c.num_type  num_type,c.ject_id subject_id,c.tag_id stage_id
                       |from (
                       |    select ec.*,prv.director_code code_id,prv.org_type type_id,prv.stage_id tag_id,prv.subject_id ject_id,
                       |        case when (ec.ext_id=0 or ec.ext_id is null) and ec.resource_tag=2 then 2
                       |             when (ec.ext_id=0 or ec.ext_id is null) and ec.resource_tag=3 then 3
                       |             when (ec.ext_id=0 or ec.ext_id is null) and ec.resource_tag=4 then 4
                       |             when (ec.ext_id=0 or ec.ext_id is null) and ec.resource_tag=5 then 5
                       ...
                       |			 when (ec.ext_id=0 or ec.ext_id is null) and ec.resource_tag=11 then 13
                       |             else 0 end num_type
                       |   from (select director_code,org_type,stage_id,subject_id,version_id from  test.link_resource_platform_version_config
                       |lateral view explode(split(version_ids,',')) t as version_id
                       |where director_type=2) prv
                       |join  neworiental_v3.entity_teaching_directory etd on etd.version_id =prv.version_id
                       |join  neworiental_v3.entity_teaching_chapter etc on etc.directory_id=etd.id
                       ...
                       |union all
                       |select 3 type, t.code_id code_id,0 org_type,count(distinct t.num_id) num,
                       |    count(distinct case when share_time>=date_sub(date_sub('${DATE}',1),pmod(datediff(date_sub('${DATE}',1),'1920-01-01')-3,7)-1) and share_time<'${DATE}' then   t.num_id end) num_week,
                       |	count( distinct  case when t.share_time>=date_sub(date_sub('${DATE}',1),day('${DATE}')-2) and t.share_time<'${DATE}' then t.num_id end) month_week,
                       |    t.num_type num_type,t.subject_id subject_id,t.stage_id stage_id from
                       |(
                       ...
                       |--初始化资源库
                       |union all
                       |select 3 type,c.code_id code_id,c.type_id org_type,c.id num_id,share_time,
                       |    c.num_type  num_type,c.ject_id subject_id,c.tag_id stage_id
                       |from (
                       |    select ec.*,prv.director_code code_id,prv.org_type type_id,prv.stage_id tag_id,prv.subject_id ject_id,
                       |        case when (ec.ext_id=0 or ec.ext_id is null) and ec.resource_tag=2 then 2
                       ...
                       |--初始化资源库
                       |union all
                       |select 4 type,c.code_id code_id,c.type_id org_type,c.id num_id,share_time,
                       |    c.num_type  num_type,c.ject_id subject_id,c.tag_id stage_id
                       |from (
                       |    select ec.*,prv.director_code code_id,prv.org_type type_id,prv.stage_id tag_id,prv.subject_id ject_id,
                       |        case when (ec.ext_id=0 or ec.ext_id is null) and ec.resource_tag=2 then 2
    ...
    val str_xiti=s""" insert into table test.resource_local_tmp
                    |select 1 type,t.code_id code_id,org_type,count(distinct t.num_id) num,
                    |    count(distinct case when share_time>=date_sub(date_sub('${DATE}',1),pmod(datediff(date_sub('${DATE}',1),'1920-01-01')-3,7)-1) and share_time<'${DATE}' then  t.num_id end) num_week,
                    |	count( distinct  case when t.share_time>=date_sub(date_sub('${DATE}',1),day('${DATE}')-2) and t.share_time<'${DATE}' then t.num_id end) month_week,
                    |    11 num_type,t.subject_id subject_id,t.stage_id stage_id from
                    |(
                    |select 1 type,lerm.org_id code_id,ee.org_type org_type,ee.id num_id,share_time,
                    ...
                    |--初始化资源库
                    |union all
                    |select 1 type,prv.org_id code_id,ee.org_type org_type,ee.id num_id,share_time,
                    |    11 num_type,ee.subject_id subject_id,ee.stage_id stage_id
                    |	from (select org_id,org_type,stage_id,subject_id,version_id from  test.link_resource_platform_version_config
                    |lateral view explode(split(version_ids,',')) t as version_id
                    |where director_type=1) prv
                    ...
                    |--初始化资源库
                    |union all
                    |select 2 type,prv.director_code code_id,ee.org_type org_type,ee.id num_id,share_time,
                    |    11 num_type,ee.subject_id subject_id,ee.stage_id stage_id
                    |	from (select director_code,org_type,stage_id,subject_id,version_id from  test.link_resource_platform_version_config
                    |lateral view explode(split(version_ids,',')) t as version_id
                    |where director_type=2) prv
                    ...
                    |--初始化资源库
                    |union all
                    |select 3 type,prv.director_code code_id,ee.org_type org_type,ee.id num_id,share_time,
                    |    11 num_type,ee.subject_id subject_id,ee.stage_id stage_id
                    |	from (select director_code,org_type,stage_id,subject_id,version_id from  test.link_resource_platform_version_config
                    |lateral view explode(split(version_ids,',')) t as version_id
                    |where director_type=3) prv
                    ...
                    |--初始化资源库
                    |union all
                    |select 4 type,prv.director_code code_id,ee.org_type org_type,ee.id num_id,share_time,
                    |    11 num_type,ee.subject_id subject_id,ee.stage_id stage_id
                    |	from (select director_code,org_type,stage_id,subject_id,version_id from  test.link_resource_platform_version_config
                    |lateral view explode(split(version_ids,',')) t as version_id
                    |where director_type=4) prv
