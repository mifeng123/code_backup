...
object DataQuality {
 ...
 /*   val str_drop_dqa="""  drop table  test.data_quality_autonomy """.stripMargin
    ...
    val str_create_dqc=s""" create table test.data_quality_class as
                          |select cast(concat('${DAY}',row_number() over()) as bigint) id,h1.* from
                          |(select distinct h.org_id,h.grade_id,h.class_id,2 public_private,h.subject_id,h.all_num answer_num,h.right_num,h.pad_class is_pad,h.time from
                          |(	select org_id,0 grade_id,0 subject_id,class_id,9999 pad_class,
                          |	count(question_id) all_num,
                          |	sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end) right_num,
                          |	time from test.school_weekly_question
                          ...
                          |	union all
                          |	-----所有年级，pad
                          |	select org_id,0 grade_id,0 subject_id,class_id,pad_class,
                          |	count(question_id) all_num,
                          |	sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end) right_num,
                          |	time from test.school_weekly_question
                          ...
                          |	union all
                          |	--各个年级
                          |	select org_id,grade_id,0 subject_id,class_id,9999 pad_class,
                          |	count(question_id) all_num,
                          |	sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end) right_num,
                          |	time from test.school_weekly_question
                          ...
                          |	union all
                          |	--各个年级，pad
                          |	select org_id,grade_id,0 subject_id,class_id,pad_class,
                          |	count(question_id) all_num,
                          |	sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end) right_num,
                          |	time from test.school_weekly_question
                          ...
                          |	union all
                          |	---各个年级学科
                          |	select org_id,grade_id,subject_id,class_id,9999 pad_class,
                          |	count(question_id) all_num,
                          |	sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end) right_num,
                          |	time from test.school_weekly_question
                          |	group by time,org_id,grade_id,subject_id,class_id
                          |	union all
                          |	---各个年级学科，pad
                          |	select org_id,grade_id,subject_id,class_id,pad_class,
                          |	count(question_id) all_num,
                          |	sum(case when ret_num=1 then 1
                          |	         when ret_num=5 then 0.5 else 0 end) right_num,
                          |	time from test.school_weekly_question
                          |	group by time,org_id,grade_id,subject_id,pad_class,class_id
                          |) h) h1 """.stripMargin
    sql(str_create_dqc)
    ...
    val str_create_dqd=s""" create table test.data_quality_difficulty as
                          |select cast(concat('${DAY}',row_number() over()) as bigint) id,h1.* from
                          |(   select distinct h.org_id,h.grade_id,h.class_id,2 public_private,h.subject_id,
